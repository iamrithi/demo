...
import { CanteenData, FaqTypeData, GrievanceTypeData, UserData } from "@/types";
...
import { getAllGrievanceType } from "@/action-data/grievanceTypeAction";
const GrievanceTypeDataList = () => {
  const { data, isLoading, isError } = useQuery({
    queryKey: ["grievanceType"],
    queryFn: async () => {
      const data = await getAllGrievanceType();

      return data.data as GrievanceTypeData[];
    },
    staleTime: 0,
  });
...
export default GrievanceTypeDataList;
